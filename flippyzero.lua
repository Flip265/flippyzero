...
local parent = nil;
local objects = {
    ["Instance0"] = Instance.new("ScreenGui"); -- Flippyzero
...
do -- Set properties
    ...
    objects["Instance0"]["Name"] = "Flippyzero";
    ...
    objects["Instance24"]["Text"] = "Flippyzero";
...
return {
    { "1.0", "Flippyzero was released as CSpy (Cherry's Spy), having simple settings, remote & event spy." },
    { "1.1", "CSpy been renamed to Flippyzero, added Script scanner & http spy." },
...
return function(shared, page)
    ...
    codeBox.Text = "Welcome to Flippyzero - Event Spy!\n-- Waiting for Bindables --"
    ...
    __newindex = function(self, index, value)
        ...
        if auto and auto[2] then
            if files then
                ...
                task.spawn(getfenv().writefile, "Flippyzero/Settings.json", http:JSONEncode(saveFile))
            end
        end
    end
})
...
if files then
    getfenv().makefolder("Flippyzero")

    table.insert(settings, 1, {"Auto_load_settings", false, function(state)
        task.spawn(getfenv().writefile, "Flippyzero/Settings.bool", state and "1" or "0")

        if state then
            local success, json = pcall(getfenv().readfile, "Flippyzero/Settings.json")
            if success then
    ...
    local success, value = pcall(getfenv().readfile, "Flippyzero/Settings.bool")
    if success then
...
return function(shared, page)
    ...
    codeBox.Text = "Welcome to Flippyzero - HTTP Spy!\n-- Waiting for HTTP requests --"
...
return function(shared, page)
    ...
    codeBox.Text = "Welcome to Flippyzero - Remote Spy!\n-- Waiting for Remotes --"
...
for _, v in %s() do
    if typeof(v) == "table" then
        if v[%s] == %s then
            ...
            game:GetService("StarterGui"):SetCore("SendNotification", { Title = "Flippyzero", Text = "Unable to generate table\nDo it yourself :trolling:" })
        end
    end)
    ...
    cons[#cons + 1] = page.Contents.View.Editor:FindFirstChild("Parent").MouseButton1Click:Connect(function()
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", { Title = "Flippyzero", Text = "Parent table not found" })
    end)
